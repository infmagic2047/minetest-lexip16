...
def add(name, colors, index, alpha=None):
    if isinstance(colors, str):
        colors = (colors,)
    env.Palette("build/palette/{}.ppm".format(name), ["color/{}.ppm".format(color) for color in colors])
    env.RGB("build/rgb/{}.ppm".format(name), ["index/{}.pgm".format(index), "build/palette/{}.ppm".format(name)])
    if alpha is None:
        env.PNG("build/png/{}.png".format(name), "build/rgb/{}.ppm".format(name))
    else:
        env.PNG("build/png/{}.png".format(name), ["build/rgb/{}.ppm".format(name), "alpha/{}.pgm".format(alpha)])
    env.Optimize("output/{}.png".format(name), "build/png/{}.png".format(name))
    add_output("{}.png".format(name))


def copy(name, othername):
    env.CopyFile("output/{}.png".format(name), "output/{}.png".format(othername))
    add_output("{}.png".format(name))


def get_texture_name(texture):
    return "{}_override{}".format(PROJECT_NAME, texture) if texture[0] == "_" else texture
...
def get_texture_string(parts):
    return "^".join(
        (get_texture_name(part) + ".png" if part[0] != "[" else part)
...
def add_overrides(overrides):
    ...
    for (nodename, face), texture in sorted(overrides.items()):
        lines.append("{} {} {}".format(nodename, face, get_texture_string((texture,) if isinstance(texture, str) else texture)))
