...
_pipeworks_mese_tube_overrides_left = {
    ("pipeworks:mese_tube_{:d}{:d}{:d}{:d}{:d}{:d}".format(left, right, bottom, top, front, back), "left"): (
        "pipeworks_mese_tube_plain_1",
    )
    + (("_pipeworks_mese_tube_plain_right_overlay",) if not front and (bottom or top) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR90"),) if not top and (front or back) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR180"),) if not back and (bottom or top) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR270"),) if not bottom and (front or back) else ())
...
_pipeworks_mese_tube_overrides_right = {
    ("pipeworks:mese_tube_{:d}{:d}{:d}{:d}{:d}{:d}".format(left, right, bottom, top, front, back), "right"): (
        "pipeworks_mese_tube_plain_2",
    )
    + (("_pipeworks_mese_tube_plain_right_overlay",) if not back and (bottom or top) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR90"),) if not top and (front or back) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR180"),) if not front and (bottom or top) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR270"),) if not bottom and (front or back) else ())
    for left in (False, True)
    for right in (False, True)
    for bottom in (False, True)
...
_pipeworks_mese_tube_overrides_bottom = {
    ("pipeworks:mese_tube_{:d}{:d}{:d}{:d}{:d}{:d}".format(left, right, bottom, top, front, back), "bottom"): (
        "pipeworks_mese_tube_plain_3",
    )
    + (("_pipeworks_mese_tube_plain_right_overlay",) if not right and (front or back) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR90"),) if not front and (left or right) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR180"),) if not left and (front or back) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR270"),) if not back and (left or right) else ())
    for left in (False, True)
    for right in (False, True)
    for bottom in (False, True)
    for top in (False, True)
...
_pipeworks_mese_tube_overrides_top = {
    ("pipeworks:mese_tube_{:d}{:d}{:d}{:d}{:d}{:d}".format(left, right, bottom, top, front, back), "top"): (
        "pipeworks_mese_tube_plain_4",
    )
    + (("_pipeworks_mese_tube_plain_right_overlay",) if not right and (front or back) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR90"),) if not back and (left or right) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR180"),) if not left and (front or back) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR270"),) if not front and (left or right) else ())
    for left in (False, True)
    for right in (False, True)
    for bottom in (False, True)
    for top in (False, True)
...
_pipeworks_mese_tube_overrides_front = {
    ("pipeworks:mese_tube_{:d}{:d}{:d}{:d}{:d}{:d}".format(left, right, bottom, top, front, back), "front"): (
        "pipeworks_mese_tube_plain_5",
    )
    + (("_pipeworks_mese_tube_plain_right_overlay",) if not right and (bottom or top) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR90"),) if not top and (left or right) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR180"),) if not left and (bottom or top) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR270"),) if not bottom and (left or right) else ())
    for left in (False, True)
    for right in (False, True)
    for bottom in (False, True)
    for top in (False, True)
...
_pipeworks_mese_tube_overrides_back = {
    ("pipeworks:mese_tube_{:d}{:d}{:d}{:d}{:d}{:d}".format(left, right, bottom, top, front, back), "back"): (
        "pipeworks_mese_tube_plain_6",
    )
    + (("_pipeworks_mese_tube_plain_right_overlay",) if not left and (bottom or top) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR90"),) if not top and (left or right) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR180"),) if not right and (bottom or top) else ())
    + ((("_pipeworks_mese_tube_plain_right_overlay", "[transformR270"),) if not bottom and (left or right) else ())
    for left in (False, True)
    for right in (False, True)
    for bottom in (False, True)
...
textures = {
    ...
    "_pipeworks_crossing_tube_plain_right_overlay": (
        "plastic",
        "crossing_tube_plain_right_overlay",
        "tube_plain_right_overlay",
    ),
    ...
    "pipeworks_mese_tube_noctr_1": (
        ("mese", "color_black", "color_green", "color_white", "color_yellow"),
        "mese_tube_noctr",
        "mese_tube_noctr",
    ),
    "pipeworks_mese_tube_noctr_2": (
        ("mese", "color_white", "color_green", "color_black", "color_yellow"),
        "mese_tube_noctr",
        "mese_tube_noctr",
    ),
    "pipeworks_mese_tube_noctr_3": (
        ("mese", "color_blue", "color_black", "color_red", "color_white"),
        "mese_tube_noctr",
        "mese_tube_noctr",
    ),
    "pipeworks_mese_tube_noctr_4": (
        ("mese", "color_blue", "color_white", "color_red", "color_black"),
        "mese_tube_noctr",
        "mese_tube_noctr",
    ),
    "pipeworks_mese_tube_noctr_5": (
        ("mese", "color_blue", "color_green", "color_red", "color_yellow"),
        "mese_tube_noctr",
        "mese_tube_noctr",
    ),
    "pipeworks_mese_tube_noctr_6": (
        ("mese", "color_red", "color_green", "color_blue", "color_yellow"),
        "mese_tube_noctr",
        "mese_tube_noctr",
    ),
    "pipeworks_mese_tube_plain_1": (
